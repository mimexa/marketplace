...
import static paris.velocafe.marketplace.utils.Constantes.Colonnes.Produits.*;
...
public class ProduitDbRowMapper extends AbstractRowMapper<ProduitDb> {
	...
	public ProduitDb mapRow(ResultSet rs, int line) throws SQLException {
		...
		produitDb.setDimensionCadre(rs.getString(DIM_CADRE_PRODUIT)); // INT
		produitDb.setDisponibilite(rs.getString(DISPO_PRODUIT));
		produitDb.setUserSize(rs.getString(USER_SIZE_PRODUIT)); // ADD COLUMN `USER_SIZE` INT NULL COMMENT '' AFTER `DISPO`,
